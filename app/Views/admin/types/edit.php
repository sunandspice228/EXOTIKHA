...
<div class="max-w-xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        <a href="<?php echo URLROOT; ?>/admin/types" class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-slate-800 transition">
            <i class="fas fa-arrow-left"></i>
        </a>
        <h1 class="text-2xl font-bold text-slate-800"><?php echo lang('title_edit_type'); ?></h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <form action="<?php echo URLROOT; ?>/admin/types_edit/<?php echo $data['type']->id; ?>" method="POST">
            <div class="space-y-6">
                
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-2 uppercase">Type Name (EN) *</label>
                    <input type="text" name="name" id="name_en" required value="<?php echo $data['type']->name; ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary">
                </div>
                
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label class="block text-xs font-bold text-indigo-600 uppercase">Nom du Type (FR)</label>
                    ...
                    <div class="relative">
                        <input type="text" name="name_fr" id="name_fr" value="<?php echo $data['type']->name_fr; ?>" class="w-full bg-indigo-50/30 border border-indigo-100 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500">
