...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        ...
        <h1 class="text-2xl font-bold text-slate-800"><?php echo lang('title_edit_post'); ?></h1>
    </div>

    <form action="<?php echo URLROOT; ?>/admin/blog_edit/<?php echo $data['post']->id; ?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 gap-4 mb-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title_en" required value="<?php echo $data['post']->title; ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary font-bold text-lg">
                        </div>
                        <div class="relative">
                            ...
                            <input type="text" name="title_fr" id="title_fr" value="<?php echo $data['post']->title_fr; ?>" class="w-full bg-indigo-50/30 border border-indigo-100 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500 font-bold text-lg">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="content" id="content_en" rows="10" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary"><?php echo $data['post']->content; ?></textarea>
                        </div>
                        <div class="relative">
                            <label class="block text-xs font-bold text-indigo-600 mb-2 uppercase flex justify-between">
                                Contenu (FR)
                            ...
                            <textarea name="content_fr" id="content_fr" rows="10" class="w-full bg-indigo-50/30 border border-indigo-100 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500"><?php echo $data['post']->content_fr; ?></textarea>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:border-primary transition relative">
                            <?php if(!empty($data['post']->image)): ?>
                                <img src="<?php echo URLROOT . '/uploads/' . $data['post']->image; ?>" class="w-full h-32 object-cover rounded mb-2" id="currentImg">
                            <?php endif; ?>
    ...
    function previewImage(input, id) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if(document.getElementById('currentImg')) document.getElementById('currentImg').classList.add('hidden');
            }
