...
<div class="relative h-[400px] md:h-[500px] flex items-center justify-center overflow-hidden bg-slate-900">
    <div class="absolute inset-0 z-0">
        <?php if(!empty($data['post']->image)): ?>
            <img src="<?php echo URLROOT . '/img/' . $data['post']->image; ?>" class="w-full h-full object-cover opacity-50 blur-sm scale-105">
        <?php else: ?>
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-6 text-center text-white animate-fade-in-up">
        <span class="inline-block px-3 py-1 bg-accent text-white text-[10px] font-black uppercase tracking-widest rounded mb-4 border border-white/30 backdrop-blur-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-slate-100">
                <?php if(!empty($data['post']->image)): ?>
                    <img src="<?php echo URLROOT . '/img/' . $data['post']->image; ?>" class="w-full rounded-xl mb-8 shadow-sm">
                <?php endif; ?>
                ...
                <div class="mt-12 pt-8 border-t border-slate-100 flex items-center justify-between">
                    <span class="font-bold text-slate-900 text-sm">Share this article</span>
        ...
        <div class="space-y-8">
            <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                <h3 class="font-serif font-bold text-slate-900 mb-4 text-lg">About Exotikha</h3>
                <p class="text-sm text-slate-500 leading-relaxed mb-4">
                    We celebrate modern African fashion with a touch of boldness and elegance. Discover our exclusive collections designed for authenticity.
                </p>
                <a href="<?php echo URLROOT; ?>/shop" class="block w-full text-center bg-slate-900 text-white py-3 rounded-xl font-bold text-sm hover:bg-primary transition">Visit the Shop</a>
            </div>

            <div>
                <h3 class="font-serif font-bold text-slate-900 mb-4 text-lg">Read Also</h3>
                <div class="space-y-4">
                    <?php if(!empty($data['recent'])): ?>
                        <?php foreach($data['recent'] as $recent): ?>
                            <?php if($recent->id != $data['post']->id): // Don't show current post ?>
                            <a href="<?php echo URLROOT; ?>/pages/post/<?php echo $recent->id; ?>" class="flex gap-4 group">
                                <div class="w-20 h-20 rounded-lg overflow-hidden bg-slate-200 flex-shrink-0">
                                    <?php if($recent->image): ?>
                                        <img src="<?php echo URLROOT . '/img/' . $recent->image; ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                                    <?php endif; ?>
                                </div>
                                <div>
                                    ...
                                    <h4 class="font-bold text-slate-800 text-sm leading-tight group-hover:text-primary transition mt-1">
