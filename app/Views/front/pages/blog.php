...
<div class="bg-slate-50 py-16 border-b border-slate-200">
    <div class="max-w-4xl mx-auto px-6 text-center">
        <span class="text-primary font-black uppercase tracking-widest text-xs mb-4 block animate-fade-in-down"><?php echo lang('blog_subtitle'); ?></span>
        <h1 class="text-4xl md:text-6xl font-serif font-bold text-slate-900 mb-6"><?php echo lang('blog_title'); ?></h1>
        <p class="text-slate-500 text-lg leading-relaxed max-w-2xl mx-auto">
            <?php echo lang('blog_intro'); ?>
        </p>
    </div>
</div>
...
<div class="max-w-7xl mx-auto px-6 py-16">
    ...
    <?php if(empty($data['posts'])): ?>
        <div class="text-center py-20 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50">
            <p class="text-slate-400 italic mb-2 text-xl"><?php echo lang('blog_empty'); ?></p>
            <p class="text-xs font-bold text-slate-300 uppercase tracking-wider">Check back soon!</p>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <?php foreach($data['posts'] as $post): ?>
                <?php 
                    // Titre et Contenu Bilingue
                    $postTitle = ($lang == 'fr' && !empty($post->title_fr)) ? $post->title_fr : $post->title;
                    $postContent = ($lang == 'fr' && !empty($post->content_fr)) ? $post->content_fr : $post->content;
                ...
                <article class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition duration-300 group h-full flex flex-col border border-slate-100">
                    ...
                    <div class="p-8 flex-1 flex flex-col">
                        ...
                        <h3 class="text-2xl font-bold mb-3 text-slate-900 leading-tight group-hover:text-primary transition font-serif">
                        ...
                        <p class="text-slate-500 text-sm mb-6 line-clamp-3 leading-relaxed">
                            <?php echo substr(strip_tags($postContent), 0, 120); ?>...
                        </p>
                        ...
                        <div class="mt-auto pt-6 border-t border-slate-50 flex justify-between items-center">
                            <a href="<?php echo URLROOT; ?>/pages/post/<?php echo $post->id; ?>" class="text-xs font-bold uppercase tracking-wider text-slate-900 group-hover:text-primary transition flex items-center gap-2">
                                <?php echo lang('blog_read_article'); ?> <i class="fas fa-arrow-right group-hover:translate-x-1 transition"></i>
                            </a>
                        </div>
                    </div>
                </article>
