...
<div class="min-h-[75vh] flex items-center justify-center bg-slate-50 relative overflow-hidden py-12 px-6">
    ...
    <div class="max-w-md w-full bg-white p-10 rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-100 relative z-10">
        ...
        <div class="text-center mb-8">
            <h2 class="text-2xl font-serif font-bold text-slate-900 mb-2">Password Recovery</h2>
            <p class="text-sm text-slate-500 leading-relaxed">
                Don't worry, it happens. Enter your email address below and we'll send you instructions to reset your password.
            </p>
        </div>
        ...
        <form action="<?php echo URLROOT; ?>/users/forgot_password" method="POST" class="space-y-6">
            ...
            <?php if(!empty($data['error'])): ?>
                <div class="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-xs font-bold text-center flex items-center justify-center gap-2">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" value="<?php echo $data['email']; ?>" class="w-full pl-10 pr-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-accent focus:border-transparent transition placeholder-slate-400 text-sm font-medium" required placeholder="name@example.com">
                </div>
            </div>
        ...
        <div class="mt-8 text-center border-t border-slate-100 pt-6">
            <p class="text-slate-400 text-xs mb-2">Remember your password?</p>
            <a href="<?php echo URLROOT; ?>/users/login" class="inline-flex items-center gap-2 text-sm font-bold text-slate-900 hover:text-accent transition">
