...
<div class="min-h-[80vh] flex items-center justify-center bg-slate-50 relative overflow-hidden py-12 px-6">
    ...
    <div class="max-w-md w-full bg-white p-10 rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-100 relative z-10">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-sm text-slate-500">Please sign in to your account</p>
        </div>
        ...
        <form class="space-y-6" action="<?php echo URLROOT; ?>/users/login" method="POST">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input name="email" type="email" value="<?php echo $data['email']; ?>" class="w-full pl-10 pr-4 py-3.5 rounded-xl border <?php echo (!empty($data['email_err'])) ? 'border-red-500 bg-red-50' : 'border-slate-200 bg-slate-50'; ?> focus:bg-white focus:ring-2 focus:ring-accent focus:border-transparent transition placeholder-slate-400 text-sm font-medium" placeholder="name@example.com" required>
                </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input name="password" type="password" value="<?php echo $data['password']; ?>" class="w-full pl-10 pr-4 py-3.5 rounded-xl border <?php echo (!empty($data['password_err'])) ? 'border-red-500 bg-red-50' : 'border-slate-200 bg-slate-50'; ?> focus:bg-white focus:ring-2 focus:ring-accent focus:border-transparent transition placeholder-slate-400 text-sm font-medium" placeholder="••••••••" required>
                </div>
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="remember-me" class="w-4 h-4 text-accent border-slate-300 rounded focus:ring-accent">
                    <span class="ml-2 text-xs font-bold text-slate-600">Remember me</span>
                </label>

                <a href="<?php echo URLROOT; ?>/users/forgot_password" class="text-xs font-bold text-accent hover:text-slate-900 transition">
                    Forgot password?
                </a>
            </div>

            <button type="submit" class="w-full bg-slate-900 text-white font-bold py-4 rounded-xl shadow-lg shadow-slate-900/20 hover:bg-accent transition transform hover:-translate-y-1 uppercase tracking-widest text-xs group">
                Sign In <i class="fas fa-arrow-right ml-2 group-hover:translate-x-1 transition-transform"></i>
            </button>
        </form>

        <div class="mt-10 text-center border-t border-slate-100 pt-6">
            <p class="text-slate-500 text-xs mb-3">Don't have an account yet?</p>
            <a href="<?php echo URLROOT; ?>/users/register" class="inline-block border-2 border-slate-100 text-slate-900 px-6 py-2 rounded-lg font-bold text-xs uppercase tracking-wide hover:border-accent hover:text-accent transition">
                Create Account
            </a>
        </div>
    </div>
